...
class SelfAttentiveCNF(nn.Module):
    ...
    def _cond_logliks(self, event_times, spatial_locations, input_mask=None, aux_state=None):
        ...
        # Only select spatial dimensions
        if D != self.dim:
            spatial_locations = spatial_locations[..., :self.dim]
            D = self.dim
    ...
    def spatial_conditional_logprob_fn(self, t, event_times, spatial_locations, aux_state=None):
        ...
        def loglikelihood_fn(s):
            bsz = s.shape[0]
            bsz_event_times = event_times[None]
            bsz_event_times = torch.cat([bsz_event_times, torch.ones(1, bsz).to(bsz_event_times) * t], dim=1)
            bsz_spatial_locations = spatial_locations[None]
            bsz_spatial_locations = torch.cat([bsz_spatial_locations, s.reshape(1, bsz, D)], dim=1)

            if aux_state is not None:
                bsz_aux_state = torch.cat([aux_state[:, :T], aux_state[:, -1].expand(1, bsz, -1)], axis=1)
            else:
                bsz_aux_state = None

            ### Below is from function _cond_logliks ###
            bsz_spatial_locations = bsz_spatial_locations.clone().requires_grad_(True)

            t_embed = self.t_embedding(bsz_event_times) / math.sqrt(self.t_embedding_dim)
            if aux_state is not None:
                inputs = [bsz_spatial_locations, bsz_aux_state, t_embed]
            else:
                inputs = [bsz_spatial_locations, t_embed]
            ...
            _x = x.reshape(T + bsz, -1)
            _bsz_event_times = bsz_event_times.transpose(0, 1).reshape(T + bsz)

            t0_cnf = _bsz_event_times + self.time_offset
            t1_cnf = torch.zeros_like(_bsz_event_times) + self.time_offset

            z, delta_logp = self.cnf.integrate(t0_cnf, t1_cnf, _x, torch.zeros_like(_bsz_event_times), norm=norm_fn)
            z = z[:, :self.dim]  # (T + N, D)

            base_t = torch.zeros_like(_bsz_event_times)
            z, delta_logp = self.base_cnf.integrate(t1_cnf, base_t, z, delta_logp)

            if aux_state is not None:
                cond_inputs = [bsz_aux_state[:, :T+1, -self.aux_dim:], t_embed[:, :T+1, :]]
            else:
                cond_inputs = [t_embed[:, :T+1, :]]
            cond = torch.cat(cond_inputs, dim=-1).reshape(T+1, -1)

            z_params = self.base_dist_params(cond)
            z_mean, z_logstd = torch.split(z_params, D, dim=-1)

            logpz1 = gaussian_loglik(z[:T], z_mean[:T], z_logstd[:T])
            logpz2 = gaussian_loglik(z[T:], z_mean[T].expand(bsz, D), z_logstd[T].expand(bsz, D))

            logpz = torch.cat([logpz1.sum(-1), logpz2.sum(-1)], axis=0)
            logpx = logpz - delta_logp  # (T + N)

            return logpx[T:] + logpx[:T].sum()

        return loglikelihood_fn
    
    
    def spatial_conditional_logprob_fn_origin(self, t, event_times, spatial_locations, aux_state=None):
        """
        Args:
            t: scalar
            event_times: (T,)
