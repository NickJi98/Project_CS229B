...
class SharedHiddenStateSpatiotemporalModel(SpatiotemporalModel, metaclass=ABCMeta):
    ...
    def __init__(self, dim=2, spatial_dim=2, hidden_dims=[64, 64, 64], tpp_hidden_dims=[8, 20], tpp_cond=False, tpp_style="split",
        ...
        self._build_spatial_model(spatial_dim, hidden_dims, actfn, zero_init, aux_dim=tpp_hidden_dims[0] // 2,
    ...
    def spatial_conditional_logprob_fn_origin(self, t, event_times, spatial_locations, t0, t1):
        hidden_state_times = torch.cat([event_times, torch.tensor(t).reshape(-1).to(event_times)]).reshape(1, -1)
        _, _, hidden_states = self.temporal_model.integrate_lambda(hidden_state_times, spatial_locations[None], input_mask=None, t0=t0, t1=None)
        hidden_states = hidden_states[:, 1:]  # Remove first (t=t0) hidden state.
        return self.spatial_model.spatial_conditional_logprob_fn_origin(t, event_times, spatial_locations, aux_state=hidden_states)
