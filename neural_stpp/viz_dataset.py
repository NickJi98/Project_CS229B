...
def plot_density(loglik_fn, spatial_locations, index, S_mean, S_std, 
                 savepath, dataset_name, device, text=None, fp64=False, N=50):
    # N = 50
...
def load_data(data, split="train", npz_file='./data/earthquakes/eq_JP.npz'):

    if data == "citibike":
        return datasets.Citibike(split=split)
    elif data == "covid_nj_cases":
        return datasets.CovidNJ(split=split)
    elif data == "earthquakes_jp" or data == 'eq_mag':
        return datasets.Earthquakes(split=split, npz_file=npz_file)
    elif data == "pinwheel":
