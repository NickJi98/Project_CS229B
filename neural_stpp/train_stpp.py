...
def setup(rank, world_size, port):
    ...
    if my_device == "gpu":
        dist.init_process_group("nccl", rank=rank, world_size=world_size, timeout=datetime.timedelta(minutes=30))  # GPU
    elif my_device == "cpu":
        dist.init_process_group("gloo", rank=rank, world_size=world_size, timeout=datetime.timedelta(minutes=30))  # CPU
...
### TODO: Check normalization of NLL ###
...
def _main(rank, world_size, args, savepath, logger):
    ...
    train_set = load_data(args.data, split="train", npz_file=args.data_file)
    val_set = load_data(args.data, split="val", npz_file=args.data_file)
    test_set = load_data(args.data, split="test", npz_file=args.data_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_file", type=str, default='./data/earthquakes/eq_JP.npz')
